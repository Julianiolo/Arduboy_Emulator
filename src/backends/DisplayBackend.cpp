...
ABB::DisplayBackend::DisplayBackend(Console* mcu, const char* name) : name(name), mcu(mcu) {
	//                                                    \/ padding
	displayImg.width = (int)(mcu->consts.DISPLAY_WIDTH   + 2);
	displayImg.height = (int)(mcu->consts.DISPLAY_HEIGHT + 2);
...
void ABB::DisplayBackend::update() {
	...
	for (size_t y = 0; y < mcu->consts.DISPLAY_HEIGHT; y++) {
		for (size_t x = 0; x < mcu->consts.DISPLAY_WIDTH; x++) {
	...
	for (size_t x = 0; x < mcu->consts.DISPLAY_WIDTH; x++) { // top edge
		((Color3*)displayImg.data)[x+1]                                            = mcu->display_getPixel(                   x,                            0).r ? lightColor : darkColor;
	}
	for (size_t x = 0; x < mcu->consts.DISPLAY_WIDTH; x++) { // bottom edge
		((Color3*)displayImg.data)[(displayImg.height-1) * displayImg.width + x+1] = mcu->display_getPixel(                   x, mcu->consts.DISPLAY_HEIGHT-1).r ? lightColor : darkColor;
	}

	for (size_t y = 0; y < mcu->consts.DISPLAY_HEIGHT; y++) { // left edge
		((Color3*)displayImg.data)[(y+1)*displayImg.width]                         = mcu->display_getPixel(                   0,                            y).r ? lightColor : darkColor;
	}
	for (size_t y = 0; y < mcu->consts.DISPLAY_HEIGHT; y++) { // right edge
		((Color3*)displayImg.data)[(y+1)*displayImg.width + displayImg.width-1]    = mcu->display_getPixel(mcu->consts.DISPLAY_WIDTH-1,                     y).r ? lightColor : darkColor;
	}
...
Rectangle ABB::DisplayBackend::getTexSrcRect() {
	return { 1,1, (float)mcu->consts.DISPLAY_WIDTH, (float)mcu->consts.DISPLAY_HEIGHT };
}
...
void ABB::DisplayBackend::draw(const ImVec2& contentSize, bool showToolTip, ImDrawList* drawList) {
	...
	ImVec2 texPos;
	{
		float ratio = (float)mcu->consts.DISPLAY_WIDTH / (float)mcu->consts.DISPLAY_HEIGHT;
...

	{
		...
		ImGuiExt::ImageRot90(ImGui::GetID("mainTex"), ImGuiExt::TextureToImTextureID(displayTex), size, rotation, uvMin, uvMax, {1,1,1,1}, {0,0,0,0}, pos, drawList);//ImVec2(!flipDims ? size.x : size.y, !flipDims ? size.y : size.x)
	}
	...
	if (showToolTip && ImGui::IsItemHovered() && ImGui::IsMouseDown(ImGuiMouseButton_Left)) {
		...
		const ImVec2 region = { 32, 16 };
		ImVec2 regionAdj = region;
		if (flipDims) std::swap(regionAdj.x, regionAdj.y);
		const ImVec2 texSize = { (float)mcu->consts.DISPLAY_WIDTH, (float)mcu->consts.DISPLAY_HEIGHT };
		...
		ImVec2 relPosReg;
		{
			...
			if (rotation == 1 || rotation == 2) relPosAdj.y = mcu->consts.DISPLAY_HEIGHT - relPosAdj.y;
			if (rotation == 2 || rotation == 3) relPosAdj.x = mcu->consts.DISPLAY_WIDTH - relPosAdj.x;
		...
		ImGuiExt::ImageRot90(ImGui::GetID("tex"), ImGuiExt::TextureToImTextureID(displayTex),
		...
		if (drawBorders) {
			...
			ImVec2 pos2 = ImGui::GetItemRectMin();
			{
				...
				float xFrac = (float)std::fmod(relPosReg.x, 1);
...

			{
				...
				float yFrac = (float)std::fmod(relPosReg.y, 1);
