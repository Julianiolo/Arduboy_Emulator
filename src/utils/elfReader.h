...
namespace utils {
	...
	public:
		struct ELFFile {
			...
			// these would both be a uint32_t on 32bit files, but here 64bit is always chosen for compadibility
			...
			std::vector<std::vector<uint8_t>> segmentContents;
	...
	private:
		uint64_t intFromByteArr(const uint8_t* data, uint8_t byteLen, bool lsb = false);

		ELFFile::ELFHeader::Ident parseELFHeaderIdentification(const uint8_t* data);
		ELFFile::ELFHeader parseELFHeader(const uint8_t* data, size_t dataLen, size_t* size);
	public:

		ELFFile parseELFFile(const uint8_t* data, size_t dataLen);
	};
}
